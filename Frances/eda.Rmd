...
ggplot(day,aes(x=NormSWTime_mu,color=age_group)) + geom_histogram() + facet_grid(Day ~ .)
ggplot(day,aes(x=NormSWDist_mu,color=age_group)) + geom_histogram() + facet_grid(Day ~ .)
```
...
This suggests that we should take into account starting memory score as well as change in memory score over the five days.
...
diffs_51 <- mwmlong %>% drop_na %>% 
  ...
  mutate(diff_dist=mu_Dist_Day5-mu_Dist_Day1,
         ...
         high_ender = (mu_Dist_Day5 >=0.6),
         high_starter=(mu_Dist_Day1 >=0.4)) %>%
  select(runno,diff_dist,diff_time,diff_pool,high_starter,high_ender)

ggplot(diffs_51,aes(x=diff_dist,fill=high_ender)) +geom_histogram() +facet_grid(.~high_starter)
ggplot(diffs_51,aes(x=diff_time,fill=high_ender)) +geom_histogram()+facet_grid(.~high_starter)
ggplot(diffs_51,aes(x=diff_pool,fill=high_ender)) +geom_histogram()+facet_grid(.~high_starter)
